...
class Math(object):
    ...
    def add(self, a, b, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method add: argument json_rpc_context is not type dict as required.')
        resp = self._call('Math.add',
                          [a, b], json_rpc_context)
        return resp[0]
 
    def bigAdd_async(self, a, b, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method bigAdd: argument json_rpc_context is not type dict as required.')
        return self._call('Math.bigAdd_async',
                          [a, b], json_rpc_context)[0]

    def bigAdd_check(self, job_id):
        resp = self._call('Math.bigAdd_check', [job_id])
        return resp[0]

    def bigAdd(self, a, b, json_rpc_context = None):
        job_id = self.bigAdd_async(a, b, json_rpc_context)
        while True:
            time.sleep(self.async_job_check_time)
            job_state = self.bigAdd_check(job_id)
