package Bio::KBase::Math::MathServer;
...
our %method_authentication = (
        'add' => 'none',
        'bigAdd' => 'required',
        'bigAdd_async' => 'required',
        'bigAdd_check' => 'required',
);

our %async_run_methods = (
        'bigAdd_async' => 'Math.bigAdd',
);

our %async_check_methods = (
        'bigAdd_check' => 'Math.bigAdd',
);
...
sub call_method {
    ...
    my $ctx = Bio::KBase::Math::MathServerContext->new($self->{loggers}->{userlog},
...
{
    # Service Math requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for Math but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = Bio::KBase::Math::MathServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service Math.");
    }
...
package Bio::KBase::Math::MathServerContext;

use strict;

=head1 NAME

Bio::KBase::Math::MathServerContext
...
package Bio::KBase::Math::MathServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = Bio::KBase::Math::MathServer->new(
